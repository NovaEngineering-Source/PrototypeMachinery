...
import github.kasuminova.prototypemachinery.api.recipe.requirement.overlay.RecipeRequirementOverlayApplier
import github.kasuminova.prototypemachinery.api.recipe.requirement.overlay.RecipeRequirementOverlayApplierRegistry
 ...
 *
 * 扩展：第三方 requirement 类型可以通过 [RecipeRequirementOverlayApplierRegistry] 注册 applier，
 * 从而让 overlay 支持更多组件类型，而无需在此处硬编码。
 */
...
public class RecipeOverlayProcessComponent(
    ...
    /** Apply overlay to a known requirement component type. Unknown types are returned as-is. */
    public fun applyTo(component: Any): Any {
        val req = component as? IdentifiedRecipeRequirementComponent ?: return component
        val data = overrides[overlayKey(req.type.id.toString(), req.id)] ?: return component

        // Registry-based dispatch: avoids hardcoding component classes here.
        val raw = RecipeRequirementOverlayApplierRegistry.get(req.type.id) ?: return component
        @Suppress("UNCHECKED_CAST")
        val applier = raw as? RecipeRequirementOverlayApplier<RecipeRequirementComponent> ?: return component

        return applier.apply(req, data)
    }
    ...
    private companion object {
        ...
        init {
            // Ensure built-in appliers are available when overlay is used.
            BuiltinAppliers
        }

        private object BuiltinAppliers {
            init {
                RecipeRequirementOverlayApplierRegistry.register(EnergyApplier)
                RecipeRequirementOverlayApplierRegistry.register(ItemApplier)
                RecipeRequirementOverlayApplierRegistry.register(FluidApplier)
            }

            private object EnergyApplier : RecipeRequirementOverlayApplier<EnergyRequirementComponent> {
                override val type: RecipeRequirementType<EnergyRequirementComponent> = RecipeRequirementTypes.ENERGY

                override fun apply(component: EnergyRequirementComponent, data: NBTTagCompound): EnergyRequirementComponent {
                    val props = applyPropertyOverlay(component.properties, data)

                    val inputPerTick = if (data.hasKey(KEY_ENERGY_INPUT_PER_TICK, Constants.NBT.TAG_LONG)) {
                        data.getLong(KEY_ENERGY_INPUT_PER_TICK)
                    } else {
                        component.inputPerTick
                    }
                    val outputPerTick = if (data.hasKey(KEY_ENERGY_OUTPUT_PER_TICK, Constants.NBT.TAG_LONG)) {
                        data.getLong(KEY_ENERGY_OUTPUT_PER_TICK)
                    } else {
                        component.outputPerTick
                    }
            ...
            private object ItemApplier : RecipeRequirementOverlayApplier<ItemRequirementComponent> {
                override val type: RecipeRequirementType<ItemRequirementComponent> = RecipeRequirementTypes.ITEM

                override fun apply(component: ItemRequirementComponent, data: NBTTagCompound): ItemRequirementComponent {
                    val props = applyPropertyOverlay(component.properties, data)

                    val inputs = if (data.hasKey(KEY_ITEM_INPUTS, Constants.NBT.TAG_LIST)) {
                        readItemKeys(data.getTagList(KEY_ITEM_INPUTS, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.inputs
                    }
                    val outputs = if (data.hasKey(KEY_ITEM_OUTPUTS, Constants.NBT.TAG_LIST)) {
                        readItemKeys(data.getTagList(KEY_ITEM_OUTPUTS, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.outputs
                    }
            ...
            private object FluidApplier : RecipeRequirementOverlayApplier<FluidRequirementComponent> {
                override val type: RecipeRequirementType<FluidRequirementComponent> = RecipeRequirementTypes.FLUID

                override fun apply(component: FluidRequirementComponent, data: NBTTagCompound): FluidRequirementComponent {
                    val props = applyPropertyOverlay(component.properties, data)

                    val inputs = if (data.hasKey(KEY_FLUID_INPUTS, Constants.NBT.TAG_LIST)) {
                        readFluidKeys(data.getTagList(KEY_FLUID_INPUTS, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.inputs
                    }
                    val outputs = if (data.hasKey(KEY_FLUID_OUTPUTS, Constants.NBT.TAG_LIST)) {
                        readFluidKeys(data.getTagList(KEY_FLUID_OUTPUTS, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.outputs
                    }
                    val inputsPerTick = if (data.hasKey(KEY_FLUID_INPUTS_PER_TICK, Constants.NBT.TAG_LIST)) {
                        readFluidKeys(data.getTagList(KEY_FLUID_INPUTS_PER_TICK, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.inputsPerTick
                    }
                    val outputsPerTick = if (data.hasKey(KEY_FLUID_OUTPUTS_PER_TICK, Constants.NBT.TAG_LIST)) {
                        readFluidKeys(data.getTagList(KEY_FLUID_OUTPUTS_PER_TICK, Constants.NBT.TAG_COMPOUND))
                    } else {
                        component.outputsPerTick
                    }
