...
/**
 * # EnergyHatchBlock - Energy Hatch Block
 * # EnergyHatchBlock - 能量仓方块
 *
 * Block implementation for energy input/output hatches.
 *
 * 能量输入/输出仓的方块实现。
 *
 * @param config The configuration for this energy hatch
 */
public class EnergyHatchBlock(
    public val config: EnergyHatchConfig
) : Block(Material.IRON) {

    init {
        val typeName = config.hatchType.name.lowercase()
        val tierLevel = config.tier.tier
        translationKey = "${PrototypeMachinery.MOD_ID}.energy_${typeName}_hatch_$tierLevel"
        setRegistryName(PrototypeMachinery.MOD_ID, "energy_${typeName}_hatch_$tierLevel")
        setHardness(3.5f)
        setResistance(10.0f)
    ...
    override fun createTileEntity(world: World, state: IBlockState): TileEntity {
        val currentConfig = HatchConfigRegistry.getEnergyHatchConfig(config.tier.tier, config.hatchType)
        return EnergyHatchBlockEntity(currentConfig)
    }
